...
const accentClasses: Record<DimensionKey, { ring: string; badge: string; card: string; quoteAccent: string; dot: string }> = {
  ...
  caption: {
    ...
    quoteAccent: 'text-violet-400/60',
    dot: 'bg-violet-400',
  },
  audio: {
...
export function AgentCard({ roast, index, variant = 'secondary', badge, deepLinkId }: AgentCardProps) {
  ...
  const scoreLevel = getScoreLevel(roast.score);
      ...
      <div className="relative z-10 p-5 sm:p-6">
        ...
        <div className="flex items-start justify-between gap-4 mb-4">
          <div className="flex items-center gap-3 min-w-0">
            <div className="flex h-11 w-11 items-center justify-center rounded-2xl border border-white/10 bg-zinc-950/80 text-xl shadow-lg shadow-black/20 shrink-0">
              {agent.emoji}
            </div>
